...
class Color:
	...
	def red(text, bold=False):
		return Color._color(str(text), 'red', bold)

	@staticmethod
	def green(text, bold=False):
		return Color._color(str(text), 'green', bold)

	@staticmethod
	def blue(text, bold=False):
		return Color._color(str(text), 'blue', bold)

	@staticmethod
	def yellow(text, bold=False):
		return Color._color(str(text), 'yellow', bold)

	@staticmethod
	def magenta(text, bold=False):
		return Color._color(str(text), 'magenta', bold)

	@staticmethod
	def cyan(text, bold=False):
		return Color._color(str(text), 'cyan', bold)

	@staticmethod
	def grey(text, bold=False):
		return Color._color(str(text), 'grey', bold)

	@staticmethod
	def white(text, bold=False):
		return Color._color(str(text), 'white', bold)

	@staticmethod
	def _color(text, color, bold):
		attributes = []
		if(bold):
			attributes.append('bold')
		return colored(str(text), color, attrs=attributes)
...
class Table:
	...
	def add_row(self, data):
		if len(data) != len(self.column_titles):
			raise Exception('invalid column count in row data: %s' % str(data))
